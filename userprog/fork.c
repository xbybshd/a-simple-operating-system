...
/* a 复制pcb所在的整个页,里面包含进程pcb信息及特级0极的栈,里面包含了返回地址, 然后再单独修改个别部分 */
   ...
   child_thread->elapsed_tricks = 0;
   child_thread->status = TASK_READY;
   child_thread->tricks = child_thread->priority;   // 为新进程把时间片充满
   child_thread->parent_pid = parent_thread->pid;
   child_thread->general_tag.prev = child_thread->general_tag.next = NULL;
   child_thread->all_list_tag.prev = child_thread->all_list_tag.next = NULL;
   block_desc_init(child_thread->u_block_desc);
   //memcpy(child_thread->u_block_desc,parent_thread->u_block_desc,DESC_CNT*sizeof(struct mem_block_desc));
/* b 复制父进程的虚拟地址池的位图 */
